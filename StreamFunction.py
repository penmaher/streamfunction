...
def cal_stream_fn(data, pres_var_name):
  ...
  assert data[pres_var_name].max() < 1100, 'Data is in Pa and not hPa, please update.' 
  ...
  for t in range(len(data['time'])): 
    for p in range(len(data[pres_var_name]) -1): 
      ...
      for l in range(len( data['lat']) -1):
        ...
        w_val_data = np.array([ omega[t,p,l],   omega[t,p+1,l],
        ...
        w_val = np.ma.masked_array(w_val_data, mask_val)
  ...
  for t in range(len(data['time'])): 
    for l in range(len(data['lat']) -1):
      ...
      for p in range(len(data[pres_var_name])-1):
        ...
        v_val_data=np.array([vcomp[t,p,l],  vcomp[t,p,l+1],
        ...
        v_val = np.ma.masked_array(v_val_data, mask_val)

        dp = (data[pres_var_name][p]-data[pres_var_name][p+1])*100   #in Pa and negative
        psi_p[t,p+1,l]=psi_p[t,p,l]+dp * fn_const_dp * 0.25*(np.nansum(v_val))
  
      for p in range(len(data[pres_var_name])-1,0,-1 ):
        v_val_rev_data=np.array([vcomp[t,p-1,l],vcomp[t,p-1,l+1],
                                 vcomp[t,p,l+1],vcomp[t,p,l]])
        mask_val=np.array([vcomp.mask[t,p-1,l],vcomp.mask[t,p-1,l+1],
                           vcomp.mask[t,p,l+1],vcomp.mask[t,p,l]])
        v_val_rev = np.ma.masked_array(v_val_rev_data, mask_val)

        dp=(data[pres_var_name][p] - data[pres_var_name][p-1])*100   #in Pa and positive
